...
% temp_err_L_rpca = zeros(MC, t_max- t_train);
temp_err_L_ncrpca = zeros(MC, t_max);
temp_err_L_rpca = zeros(MC, t_max);
...
% temp_SE_rpca = zeros(MC, ceil((t_max- t_train)/alpha) - 1);
temp_SE_ncrpca = zeros(MC, 3);
temp_SE_rpca = zeros(MC, 3);
temp_SE_gd = zeros(MC, ceil((t_max- t_train)/alpha) - 1);


t_calc = t_train + 2 * alpha -1 : alpha : t_max - t_train;
...
L_hat_ncrpca = zeros(n, t_max);
    S_hat_ncrpca = zeros(n, t_max);
    tt4 = tic;
    cc = 1;
    [L_hat_ncrpca(:, 1: t_1), ...
        S_hat_ncrpca(:, 1: t_1)] = ...
        ncrpca(M(:, 1 : t_1), r_0 , 1e-6, 100);
    P_track_ncrpca{cc} = orth(L_hat_ncrpca(:, 1: t_1));
    cc = cc + 1;
    
    [L_hat_ncrpca(:, t_1 + 1 : t_2), ...
        S_hat_ncrpca(:, t_1 + 1 : t_2)] = ...
        ncrpca(M(:, t_1 + 1 : t_2), r_0, 1e-6, 100);
    
    P_track_ncrpca{cc} = orth(L_hat_ncrpca(:, t_1 + 1 : t_2));
    cc = cc + 1;
    
    [L_hat_ncrpca(:, t_2 + 1 : end), ...
        S_hat_ncrpca(:, t_2 + 1 : end)] = ...
        ncrpca(M(:, t_2 + 1 : end), r_0, 1e-6, 100);
    P_track_ncrpca{cc} = orth(L_hat_ncrpca(:, t_2 + 1 : end));
    t_ncrpca = t_ncrpca + toc(tt4);
    
%     for ii = 1 : 3
%         if(~(mod(ii + 1, alpha)))
%             %fprintf('ALtMin %d\n', ii);
%             [L_hat_ncrpca(:, ii- t_train - alpha : ii - t_train), ...
%                 S_hat_ncrpca(:, ii- t_train - alpha : ii- t_train)] = ...
%                 ncrpca(M(:, ii - alpha : ii), r_0 + r_1 + r_2, 1e-6, 100);
%             P_track_ncrpca{cc} = orth(L_hat_ncrpca(:, ii - t_train - alpha...
%                 : ii- t_train));
%             cc = cc + 1;
%         end
%     end
%     for ii = alpha + t_train : t_max - t_train
%         if(~(mod(ii + 1, alpha)))
%             %fprintf('ALtMin %d\n', ii);
%             [L_hat_ncrpca(:, ii- t_train - alpha : ii - t_train), ...
%                 S_hat_ncrpca(:, ii- t_train - alpha : ii- t_train)] = ...
...
%     end
    ...
    %%%Call to PCP
    fprintf('PCP\n');
    L_hat_rpca = zeros(n, t_max);
    S_hat_rpca = zeros(n, t_max);
    tt1 = tic;
        cc = 1;
    [L_hat_rpca(:, 1: t_1), ...
        S_hat_rpca(:, 1: t_1)] = ...
        inexact_alm_rpca(M(:, 1 : t_1), sqrt(1/n), 1e-6, 100);
    P_track_rpca{cc} = orth(L_hat_rpca(:, 1: t_1));
    cc = cc + 1;
    
    [L_hat_rpca(:, t_1 + 1 : t_2), ...
        S_hat_rpca(:, t_1 + 1 : t_2)] = ...
        inexact_alm_rpca(M(:, t_1+1 : t_2), sqrt(1/n), 1e-6, 100);
    
    P_track_rpca{cc} = orth(L_hat_rpca(:, t_1 + 1 : t_2));
    cc = cc + 1;
    
    [L_hat_rpca(:, t_2 + 1 : end), ...
        S_hat_rpca(:, t_2 + 1 : end)] = ...
        inexact_alm_rpca(M(:, t_2 +1 : end), sqrt(1/n), 1e-6, 100);
    P_track_rpca{cc} = orth(L_hat_rpca(:, t_2 + 1 : end));
    t_rpca = t_rpca + toc(tt1);

%     cc = 1;
%     for ii = alpha + t_train : t_max - t_train
%         if(~(mod(ii + 1, alpha)))
%             %fprintf('ALtMin %d\n', ii);
%             [L_hat_rpca(:, ii- t_train - alpha : ii - t_train), ...
%                 S_hat_rpca(:, ii- t_train - alpha : ii- t_train)] = ...
%                 inexact_alm_rpca(M(:, ii - alpha : ii), sqrt(1/n), 1e-6, 100);
%             P_track_rpca{cc} = orth(L_hat_rpca(:, ii - t_train - alpha...
%                 : ii- t_train));
%             cc = cc + 1;
%         end
%     end
...
%     temp_err_L_rpca(mc, :) = sqrt(mean((L(:, t_train + 1 :end) - ...
%         L_hat_rpca).^2, 1)) ./ sqrt(mean(L(:, t_train + 1 :end).^2, 1));
temp_err_L_ncrpca(mc, :) = sqrt(mean((L - ...
        L_hat_ncrpca).^2, 1)) ./ sqrt(mean(L.^2, 1));
    temp_err_L_rpca(mc, :) = sqrt(mean((L - ...
        L_hat_rpca).^2, 1)) ./ sqrt(mean(L.^2, 1));
...
%         P_hat_grasta = cell2mat(U_hat_grasta(max(t_calc(jj), 1)));
        ...
        P_hat_rpca = cell2mat(P_track_rpca(jj));
...
%                 Calc_SubspaceError(P_hat_stoc, PP1);
            ...
            temp_SE_rpca(mc, jj) = ...
                Calc_SubspaceError(P_hat_rpca, PP1);
%             temp_SE_gd(mc, jj) = ...
...
%                 Calc_SubspaceError(P_hat_stoc, PP2);
            temp_SE_rpca(mc, jj) = ...
                Calc_SubspaceError(P_hat_rpca, PP2);
%             temp_SE_gd(mc, jj) = ...
...
err_L_rpca = mean(temp_err_L_rpca, 1);
...
err_SE_rpca = mean(temp_SE_rpca, 1);
...
%     : t_max - t_train - 1)), 'r','LineWidth', 2)
% hold
% plot(2 : alpha : t_max - t_train - 2, log10(err_L_stoc...
%     (1 : alpha : t_max - t_train -1)), 'k','LineWidth', 2)
% plot(1 : alpha : t_max - t_train -1, log10(err_L...
%     (1 : alpha : t_max - t_train -1)), 'b', 'LineWidth', 2)
% % hold
% plot(1 : alpha : t_max - t_train -1, log10(err_L_pca...
%     (1 : alpha : t_max - t_train -1)), 'c', 'LineWidth', 2)
plot(1 : alpha : t_max - t_train -1, log10(err_L_ncrpca...
    (1 : alpha : t_max - t_train -1)), 'g', 'LineWidth', 2)
hold
plot(1 : alpha : t_max - t_train -1, log10(err_L_rpca...
    (1 : alpha : t_max - t_train -1)), 'k', 'LineWidth', 2)
% plot(1 : alpha : t_max - t_train -1, log10(err_L_gd...
%     (1 : alpha : t_max - t_train -1)), 'm', 'LineWidth', 2)
% plot(1 : alpha : t_max - t_train -1, log10(err_L_off...
%     (1 : alpha : t_max - t_train -1)), 'y', 'LineWidth', 2)
...
save('data/tj_bern_rotation_n1000_mc50_new.mat');
...
%plot(t_calc, log10(err_SE_rpca(1 : 18)), 'k', 'LineWidth', 2)
plot(t_calc, log10(err_SE_ncrpca), 'g', 'LineWidth', 2)
%plot(t_calc, log10(err_SE_rpca), 'k', 'LineWidth', 2)
